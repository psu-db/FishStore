...
class Utility {
 public:
  ...
  static inline uint64_t GetHashCode(uint32_t a, uint32_t b) {
    return GetHashCode((static_cast<uint64_t>(a) << 32) + b);
  }
  ...
  static inline uint64_t HashBytes(const uint16_t* str, size_t len) {
    ...
    static inline uint64_t HashBytesWithPSFID(uint16_t psf_id, const char* str, size_t len) {
      const uint64_t kMagicNum = 40343;
      uint64_t hashState = len;

      hashState = kMagicNum * hashState + psf_id;
