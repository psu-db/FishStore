...
namespace fishstore::device {
    class NullDisk;
}

namespace fishstore::core {
    using psf_len_ht_t = tsl::hopscotch_map<uint64_t, size_t>;

    size_t psfLen(const psf_len_ht_t& psf_len_ht, uint32_t psf_id, uint32_t value) {
        auto it = psf_len_ht.find(
            Utility::GetHashCode(psf_id, value)
        );
        if (psf_len_ht.end() == it)
            return 0;
        return it->second;
    }

    size_t psfLen(const psf_len_ht_t& psf_len_ht, uint32_t psf_id, adapter::StringRef value) {
        auto it = psf_len_ht.find(
    ...
    using rec_count_prefix_t = std::vector<size_t>;

    size_t recordCount(const rec_count_prefix_t& prefix, uint64_t start_addr, uint64_t end_addr) {
        assert(start_addr < end_addr && "Record count illegal start and end addresses");
        const auto s_part = std::min(static_cast<size_t>(Address{start_addr}.page()), prefix.size());
        const auto e_part = std::min(static_cast<size_t>(Address{end_addr}.page()), prefix.size());

        const size_t s_prefix = s_part == 0 ? 0 : prefix[s_part - 1];
        const size_t e_prefix = prefix[e_part];

        return e_prefix - s_prefix;
    }
    ...
    public:
        ...
        FishStore(uint64_t table_size, uint64_t log_size, const std::string& filename)
            ...
            ezpsf::initJit<std::vector<typename A::field_t>>(jit.get());
        }
    ...
    public:
        ...
        template<class SC>
        Status SkipScan(SC &context, AsyncCallback callback, uint64_t monotonic_serial_num, uint64_t start_addr,
                        uint64_t end_addr, Address chain_start);
        ...
        PsfLookup MakeEzPsf(const std::string& ez_psf_str, const std::vector<std::string>& custom_fields = {});

        Address GetHeadAddress();

        Address GetTailAddress();

        // returns the PSF info for the given id
        ezpsf::EzPsfInfo getPsfInfoById(uint32_t id);
        ...
        // hash table for mapping the keyhash => number of elements in that hash table
        psf_len_ht_t psf_len_ht;
        size_t record_count = 0;

        size_t psfLen(uint32_t psf_id, uint32_t value) {
            return psfLen(psf_len_ht, psf_id, value);
        }


        constexpr size_t recordCount() const { return record_count; }
    ...
    private:
        ...
        template<class C>
        OperationStatus InternalSkipScan(C &pending_context, uint64_t start_addr, uint64_t end_addr, Address address);
    ...
    private:
        ...
        std::unordered_map<uint32_t, ezpsf::EzPsfInfo> psf_info_map;

        // Date Stuff
        typename A::ondemand_parser_t times_parser;
        // Index represents the page, value represents the first time that page contains

    public:
        void setPsf(const ezpsf::EzPsf& psf, uint32_t inline_id) {
            inline_psf_map[inline_id].ez_eval = psf;
        }

        std::vector<std::string> page_times;

        // each index stores the number of records previously seen up to and including the records on that page
        std::vector<size_t> records_prefix_sum;

        void SetTimeField(const std::string& field) {
            times_parser.setRealTime(field);
        }

        std::pair<Address, std::string> findTime(const std::string& time) {
            // upper_bound points the first element greater than time.
            // that is, if time is exactly included in the page times, then it is on the previous page
            auto it = std::upper_bound(page_times.begin(), page_times.end(), time) - 1;
            uint32_t page_id = std::distance(page_times.begin(), it);
            return std::pair<Address, std::string>(Address{page_id, 0}, *it);
        }
    };
        ...
        while (A::HasNext(parser_ctx().parser)) {
            ...
            for (auto general_psf_id: parser_state.ptr_general_psf) {
                ...
                if (psf.lib_id == LIB_EZ_PSF) {
                    ...
                    if (!has_value) {
                        // if doesn't have vaule continue
                    } else if (psf.ret_type == ezpsf::DataType::INT64_T) {
                    ...
                    } else if (psf.ret_type == ezpsf::DataType::DOUBLE_T) {
                        auto val = new double(*(double *)&string_ref);
                        res = NullableStringRef{
                            .is_null = !has_value,
                            .need_free = true,
                            .size = sizeof(double),
                    ...
                    } else if (psf.ret_type == ezpsf::DataType::STR_T && string_ref.Length() != 0) {
                        char* payload = new char[string_ref.Length()];
                        memcpy(payload, string_ref.Data(), string_ref.Length());
                        res = NullableStringRef{
                            .is_null = !has_value,
                            .need_free = true,
                            .size = static_cast<uint32_t>(string_ref.Length()),
                            .payload = payload
                        };
                    } else {
...
tot_cpu_time += std::chrono::duration<double>(t_end - t_start).count();
    ...
    template<class D, class A>
    template<class SC>
    inline Status FishStore<D, A>::SkipScan(SC& context, AsyncCallback callback,
                                        uint64_t monotonic_serial_num,
                                        uint64_t start_addr, uint64_t end_addr, Address chain_start) {
        typedef SC scan_context_t;
        typedef PendingScanContext<SC> pending_scan_context_t;

        pending_scan_context_t pending_context{context, callback};
        OperationStatus internal_status = InternalSkipScan(pending_context, start_addr, end_addr,chain_start);
        Status status;
    ...
    inline OperationStatus FishStore<D, A>::InternalRead(C& pending_context) const {
        ...
        if (address >= head_address) {
            ...
            if (std::is_same_v<disk_t, device::NullDisk>)
                return OperationStatus::NOT_FOUND;
    ...
    #ifdef INSERT_CALLBACK
    std::function<void(Address)> insert_callback;
    #endif
    ...
    inline OperationStatus FishStore<D, A>::InternalInsert(C& pending_context) {
        ...
        if (new_address.page() == page_times.size()) {
            // new page comes in
            times_parser.Load(pending_context.payload(), pending_context.payload_size());
            page_times.emplace_back(times_parser.findRealTime());
            if (records_prefix_sum.empty())
                records_prefix_sum.emplace_back(1);
            else
                records_prefix_sum.emplace_back(records_prefix_sum.back() + 1);
        } else {
            assert(records_prefix_sum.size() > new_address.page() && "No prefix sum entry created for this address!");
            ++records_prefix_sum[new_address.page()];
        }
...
tot_memcpy_time += std::chrono::duration<double>(t_memcpy_end - t_memcpy_start).count();
    ...
    inline OperationStatus FishStore<D, A>::InternalScan(C& pending_context,
        ...
        printf("%lu was found for the address to start our scan!\n",address.control());
    ...
    template<class D, class A>
    template<class C>
    inline OperationStatus FishStore<D, A>::InternalSkipScan(C& pending_context,
                                                         uint64_t start_addr, uint64_t end_addr, Address address) {
        assert(start_addr < end_addr);
        typedef C pending_scan_context_t;
    ...
    inline OperationStatus FishStore<D, A>::InternalFullScan(C& pending_context,
        ...
        printf("To address: %lu %u, %u\n", to_address.control(), to_address.page(), start_address.page());
        ...
        while (current_page >= 0) {
            ...
            //else if (current_page == hlog.head_address.load().page()) hlog.head_flag.store(true);
            ...
            while (address < to_address) {
                ...
                //XXX: bak hack;
                //if constexpr (std::is_same_v<disk_t, device::NullDisk>)
                //if (address < hlog.head_address.load()) break;
    ...
    inline Status FishStore<D, A>::IssueAsyncIoRequest(ExecutionContext& ctx,
        ...
        AsyncIOContext io_request{
            this, pending_context.address, &pending_context,
    ...
    OperationStatus FishStore<D, A>::InternalContinuePendingRead(ExecutionContext& context,
        ...
        if (io_context.address >= hlog.begin_address.load()) {
            ...
            return (thread_ctx().version > context.version)
                       ? OperationStatus::NOT_FOUND_UNMARK
    ...
    Status FishStore<D, A>::RecoverHybridLogFromSnapshotFile() {
        ...
        for (uint32_t page = start_page; page < end_page; ++page) {
            ...
            if (page + capacity < end_page) {
                Context context{
                    hlog, checkpoint_.snapshot_file, start_page, page + capacity,
                    recovery_status
                };
                RETURN_NOT_OK(hlog.AsyncFlushPage(page, recovery_status, callback, &context));
            } else {
    ...
    Status FishStore<D, A>::RestoreHybridLog() {
        ...
        // Skip the null page.
        Address head_address = start_page == 0 ? Address{0, Constants::kCacheLineBytes} : Address{start_page, 0};
    ...
    void FishStore<D, A>::SplitHashTableBuckets() {
        ...
        if (epoch_.FinishThreadPhase(Phase::GROW_IN_PROGRESS)) {
            ...
            GlobalMoveToNextState(SystemState{
                Action::GrowIndex, Phase::GROW_IN_PROGRESS,
                thread_ctx().version
            });
        } else {
    ...
    void FishStore<D, A>::HandleSpecialPhases() {
        ...
        do {
            ...
            SystemState current_state = (previous_state == final_state) ? final_state : previous_state.GetNextState();
    ...
    if (status != Status::Ok) break
        ...
        do {
            ...
            system_state_.store(SystemState{
                Action::Recover, Phase::REST,
    ...
    template<class D, class A>
    PsfLookup FishStore<D, A>::MakeEzPsf(const std::string& ez_psf_str, const std::vector<std::string>& custom_fields) {
        ezpsf::EzPsfInfo psf_info = ezpsf::getPsf(ez_psf_str, jit.get());
        psf_irs.insert({psf_info.name, psf_info.ir});
        if (!custom_fields.empty()) {
            psf_info.fields = custom_fields;
        }
        ...
        if (ezpsf::canInline(psf_info.type)) {
            ...
            psf_info_map.emplace(inline_psf_id, std::move(psf_info));
            return lookup;
        } else {
            GeneralPSF<A> psf;
            psf.fields = field_ids;
            ...
            general_psf_map.push_back(psf);
            auto lookup = PsfLookup{static_cast<uint32_t>(general_psf_id), PsfType::GENERAL};
            named_psfs.insert({psf_info.name, lookup});

            psf_info_map.emplace(general_psf_id, std::move(psf_info));
            return lookup;
        }
    }

    template<class D, class A>
    Address FishStore<D, A>::GetHeadAddress() {
        return hlog.head_address.load();
    }

    template<class D, class A>
    Address FishStore<D, A>::GetTailAddress() {
        return hlog.GetTailAddress();
    }

    template<class D, class A>
    ezpsf::EzPsfInfo FishStore<D, A>::getPsfInfoById(uint32_t id) {
